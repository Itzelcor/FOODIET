...
-- ============================================================================
-- 6. LISTA DE PACIENTES ORDENADA POR ALTURA
...
-- ============================================================================
-- 7. CANTIDAD DE PROFESIONALES POR ESPECIALIDAD
...
-- ============================================================================
				...
				COUNT(id_profesional) AS total_profesionales
				FROM profesional
				WHERE activo = 1
...
-- ============================================================================
-- 8. PACIENTES CON ALERGIAS GRAVES
...
-- ============================================================================
-- 9. PLANES DIETÉTICOS CON MÁS DE 2500 CALORÍAS
...
-- ============================================================================
				...
				ORDER BY calorias_diarias DESC LIMIT 5;


-- ============================================================================
-- 10. FACTURAS PENDIENTES DE ALTO IMPORTE
-- Selecciona las facturas que aún no se han pagado y cuyo importe supera 
-- los 100 euros, mostrando primero las más caras.
-- ============================================================================
				SELECT id_factura, id_paciente, importe, 
                fecha_emision FROM factura
				WHERE estado_pago = 'pendiente' 
                AND importe > 100.00 ORDER BY importe DESC;


-- ============================================================================
-- 11. ALIMENTOS MÁS CALÓRICOS POR CADA TOMA
...
-- ============================================================================
				...
				FROM comida c INNER JOIN alimento a 
                ON c.id_comida = a.id_comida GROUP BY c.tipo
				HAVING AVG(a.calorias) > 500
				ORDER BY promedio_calorias DESC;


-- ============================================================================
-- 12. CITAS AGENDADAS PARA UN DÍA ESPECÍFICO 
-- Muestra la hora, el nombre del paciente y el del profesional para todas 
-- las citas programadas en una fecha concreta.
-- ============================================================================
		SELECT c.hora_cita, p.nombre AS paciente, 
		pr.nom_completo AS profesional, c.modalidad
		FROM cita c
...
-- ============================================================================
-- 13. PACIENTES QUE HAN SUPERADO EL PESO MEDIO (Subconsulta en Where)
...
-- ============================================================================
-- 14. TOTAL DE DINERO RECAUDADO POR MÉTODO DE PAGO
...
-- ============================================================================
			...
			FROM factura WHERE estado_pago = 'pagada'
			GROUP BY metodo_pago ORDER BY total_recaudado DESC;


-- ============================================================================
-- 15. PACIENTES CON MÁS DE 3 CITAS CANCELADAS
...
-- ============================================================================
			...
			FROM cita WHERE estado_cita = 'cancelada'
			GROUP BY id_paciente HAVING COUNT(id_cita) > 3
			ORDER BY citas_canceladas DESC;


-- ============================================================================
-- 16. ALIMENTOS ASIGNADOS A UN PACIENTE ESPECÍFICO
...
-- ============================================================================
-- 17. PROFESIONALES QUE TRABAJAN LOS LUNES POR LA MAÑANA
...
FROM profesional p 	INNER JOIN horario_profesional hp 
ON p.id_profesional = hp.id_profesional WHERE hp.dia_semana = 1 AND
hp.tipo = 'laboral' AND hp.hora_inicio < '14:00:00' ORDER BY hp.hora_inicio ASC;


-- ============================================================================
-- 18. MÉTRICAS FÍSICAS MÁS RECIENTES QUE ESTÁN EN RIESGO
...
-- ============================================================================
			...
			FROM registro_metrica WHERE tipo_metrica 
			IN ('imc', 'grasa_corporal') AND valor > 30.00
			ORDER BY fecha_calculo DESC LIMIT 10;
