...
-- Procedimiento: registrar nueva cita con validación
DELIMITER //
...
BEGIN
    ...
    SELECT COUNT(*) INTO v_conteo FROM pacientes WHERE id_paciente = p_id_paciente;
    IF v_conteo = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El paciente no existe';
    END IF;
    
    -- Validar que el profesional exista
    SELECT COUNT(*) INTO v_conteo FROM profesionales WHERE id_profesional = p_id_profesional;
    IF v_conteo = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El profesional no existe';
    END IF;
    
    -- Validar que no haya cita duplicada (mismo profesional, misma fecha y hora)
    ...
    IF v_conteo > 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El profesional ya tiene una cita en esa fecha y hora';
    END IF;
    
    -- Insertar la cita
    INSERT INTO citas (id_paciente, id_profesional, fecha_cita, hora_cita, modalidad, estado, motivo)
    VALUES (p_id_paciente, p_id_profesional, p_fecha, p_hora, p_modalidad, 'pendiente', p_motivo);
    
    SELECT LAST_INSERT_ID() AS id_cita_creada;
END //
DELIMITER ;

-- Procedimiento: cancelar cita y gestionar reembolso
DELIMITER //
...
BEGIN
    ...
    IF v_estado = 'cancelada' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'La cita ya está cancelada';
    END IF;
    
    IF v_estado = 'completada' THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No se puede cancelar una cita ya completada';
    END IF;
    
    -- Actualizar estado de la cita
    UPDATE citas SET estado = 'cancelada' WHERE id_cita = p_id_cita;
    
    -- Reembolsar si hay pago
    SELECT id_pago, monto INTO v_id_pago, v_monto 
    FROM pagos WHERE id_cita = p_id_cita AND estado_pago = 'pagado';
    
    IF v_id_pago IS NOT NULL THEN
        UPDATE pagos SET estado_pago = 'reembolsado' WHERE id_pago = v_id_pago;
    END IF;
END //
DELIMITER ;
...
-- Trigger: crear factura automática al marcar pago como pagado
DELIMITER //
...
BEGIN
    IF NEW.estado_pago = 'pagado' AND OLD.estado_pago != 'pagado' THEN
        IF NEW.numero_factura IS NULL THEN
            SET NEW.numero_factura = CONCAT('FAC-', YEAR(NOW()), '-', LPAD(NEW.id_pago, 4, '0'));
        END IF;
...
CREATE INDEX idx_planes_paciente ON planes_alimentacion(id_paciente);
